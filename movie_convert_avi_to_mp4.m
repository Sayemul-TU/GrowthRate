...
folder = fileparts(which('Change in r.avi')); % Determine where demo folder is (works with all versions).
% Pick one of the two demo movies shipped with the Image Processing Toolbox.
% Comment out the other one.
movieFullFileName = fullfile(folder, 'Change in r.avi');
...
try
	...
	outputFileName = fullfile(outputFolder, 'Change in r.mp4');
